...
$\left|g_{a\gamma\gamma}\right|\gtrsim 8.2\times 10^{-14}\,\text{Ge\hspace{-.08em}V}^{-1}$, a factor of eleven above the benchmark 
...
temperature. In the presence of an external magnetic field, the ambient 
...
region of 10.7126--10.7186$\,\mu\text{e\hspace{-.08em}V}$~\cite{CAPPI}.
...
the attenuator anchored to the mixing flange and the transmission of the noise
 from the cavity body itself. The symbol 
...
During the data taking, the cavity sits in the center of the magnet bore 
and is connected via holders to the mixing flange of the DR. 
...
the axial direction to reduce the loss from the 
seam~\cite{TASEHInstrumentation}. 
...
Over the frequency range of the CD102 run, the form factor $C_{010}$ 
varies from 0.60 to 0.61 and 
...
line width), rather than a fixed value, such that the rotation angle of the 
tuning rod did not need to be fine-tuned and the operation time could be 
...
of TASEH. The definitions of the parameters can be found in 
Sec.~\ref{sec:intro}. 
...
$\left|g_{a\gamma\gamma}\right|$ range from $5.3\times 10^{-14}\,\text{Ge\hspace{-.08em}V}^{-1}$ to $8.9\times 10^{-14}\,\text{Ge\hspace{-.08em}V}^{-1}$, 
with an average 
value of $8.2\times 10^{-14}\,\text{Ge\hspace{-.08em}V}^{-1}$; the lowest 
...
be $\approx 4.9\times 10^{-14}\,\text{Ge\hspace{-.08em}V}^{-1}$. 
...
\begin{figure*} [htbp]
 ...
 CAPP, and HAYSTAC Collaborations. The gray band indicates the allowed region 
...
axion-two-photon coupling $\left|g_{a\gamma\gamma}\right|\gtrsim 8.2\times 10^{-14}\,\text{Ge\hspace{-.08em}V}^{-1}$ at 95\% C.L.,
...
\bibitem [{\citenamefont {Chang}\ \emph {et~al.}(2022)\citenamefont {Chang},
  ...
  {2022})},\ \Eprint {https://arxiv.org/abs/2205.01477} {arXiv:2205.01477
