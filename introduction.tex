...
results. For a Maxwell-Boltzmann velocity 
...
used by other axion experiments. Equation~\eqref{eq:simplesignal} 
...
(i) 
$T_\text{cn}=\left(\frac{1}{e^{\left.hf\middle/k_BT_\mathrm{c}\right.}-1} + \frac{1}{2}\right)\left. hf\middle/k_B\right.$, the sum of the blackbody 
...
vacuum, which is further modulated by a Lorentzian function due to the 
...
The Lorentzian modulation of $T_\text{cn}$ will be removed from the mean of 
the averaged noise spectrum and only the baseline \tsys\ will be 
used in the final analysis (Sec.~\ref{sec:ana}). The fluctuation of the 
averaged noise spectrum near resonace is $\approx 5$\% larger 
than the off-resonance regions; this 5\% difference has little effect 
on the final analysis since the on-resonance region contributes $<10\%$ to 
the full spectrum.  
...
the baseline values of $T_\text{cn}$ and \ta\ near the cavity resonant 
frequency are estimated to be about 0.19~K and \noise~K, respectively.  
