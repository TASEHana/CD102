...
\begin{equation}
    ...
    \delta_{g}^\text{merged} = \frac{ \sum_{n = 1}^{M}\delta_{g+n-1}^{c} \cdot {w_{g+n-1}}}{\sum_{n = 1}^{M} {w_{g+n-1}}}
\end{equation}

\begin{equation}
    \label{eq:merged_sigma}
    \sigma_{g}^\text{merged} =  \frac{ \sqrt{\sum_{n = 1}^{M} (\sigma_{g+n-1}^{c})^2 \cdot {w_{g+n-1}^2}}}{\sum_{n = 1}^{M} {w_{g+n-1}}}
\end{equation}

\begin{equation}
    \label{eq:merged_snr}
    \text{SNR}_{g}^\text{merged} = \frac{\delta^\text{merged}_{g}}{\sigma^\text{merged}_{g}} = \frac{\sum_{n = 1}^{M}\delta_{g+n-1}^{c} \cdot {w_{g+n-1}}}{ \sqrt{\sum_{n = 1}^{M} (\sigma_{g+n-1}^{c})^2 \cdot {w_{g+n-1}^2}}}
\end{equation}

where $g = 1,..,N-M+1$ is the number of bins in the final spectrum and N is the number of bins in the combined spectrum.
