...
%top of the noise data and found that they were consistent with the original 
%choice (see Sec.~\ref{sec:sys}). 
...
%Therefore, 
%If a signal appears in a certain frequency bin $j$, due to the
...
% The weight for each bin $j$ of the $i^\text{th}$ rescaled spectrum, 
%of which the RDP will be included in the $n^\text{th}$ bin of the combined 
%spectrum, is defined below: %in Eq.~\eqref{eq:weight}: 
...
%The value of $m_n$ ranges from 2 to 26; 
...
%and the results of the \gagg\ limits are found to be
...
%greater or equal to 5 after rescanning. 
The signals in the frequency range 4.74730 -- 4.74738~GHz 
...
signals in the frequency range 4.71017 -- 4.71019~GHz 
were not detected outside the DR but 
still present after turning off the external magnetic field. 
Therefore, the signals in these two frequency ranges are excluded from 
consideration of axion signal candidates and no limits are placed.  
