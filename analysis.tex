...
Before doing this, 
the normalized spectrum from each scan is rescaled by $R_\text{scale}$, 
and the rescaled spectrum, shown in 
Fig.~\ref{fig:rescaled_power}, is computed with the following formula:
\begin{equation}
  \label{eq:respower_eqn}
  \delta_{ij}^\text{res} = R_\text{scale}\delta_{ij}^\text{norm},
\end{equation}
and the standard deviation of each bin is:
\begin{equation}
  \label{eq:ressigma_eqn}
  \sigma_{ij}^\text{res} = R_\text{scale}\sigma_{i}^\text{norm},
\end{equation}
where 
 \begin{equation}
 R_\text{scale} = \frac{k_{B}\tsys \Delta f_\text{bin} }{P_{ij}^\text{KSVZ} h_{ij}}.
 \label{eq:Rratio}
 \end{equation}
...
The factor $R_\text{scale}$ is the ratio of 
