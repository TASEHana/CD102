...
Right plot: Histogram of the normalized spectrum (lower panel in left plot) with Gaussian fit.
...
In order to get misalignment-independent line shape, the average ($\bar{L}_{k}$) of
$L_{k}(\delta f_m)$ over the range $\delta f_m$ is used instead of $L_{k}(\delta f_m)$.
In our analysis, $\bar{L}_{k} = 0.23, 0.33, 0.21, 0.11, 0.06$ for bin index from 1 to 5.
